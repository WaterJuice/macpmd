...
- **launchd fix command** — `fix` reinstalls missing launchd plists for running processes
- **TCC path protection** — `--sudo` processes are blocked from using TCC-protected directories (Desktop, Documents, Downloads) that would fail when run as LaunchDaemons
- **Sudo support** — `--sudo` flag to run processes with elevated privileges via LaunchDaemons
